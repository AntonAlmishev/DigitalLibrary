...
public class MySecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // Позволява хоме, но изисква логин за дипбоарда
        /*
        http
                .authorizeRequests()
                .antMatchers("/dishBoard").authenticated()
     ...
     */
        // Позволяваме всичйо от юсер и забраняваме всичко от админ
        /*
        http.authorizeRequests()
                .antMatchers("/user/**").permitAll()
                .antMatchers("/ad/**").denyAll()
                .and()
                .httpBasic();

         */
        // Позволява метода от юсер/хоме, за юсердашборд изисква удостоворяване
...
/*
      //Мениджъра по удостоверяването
    @Override
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.inMemoryAuthentication()
                .withUser("Anton Almishev").password("111").authorities("admin")
                .and()
                .withUser("Ivailo Vatev").password("222").authorities("user")
                .and()
